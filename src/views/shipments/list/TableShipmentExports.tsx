...
import { getExports } from 'src/redux/slices/shipments'
...
const TableShipmentExports = () => {
  const dispatch = useAppDispatch()
  const router = useRouter()
  const { isLoading, shipmentsExport, error } = useAppSelector((state) => state.shipments)

  const auth = useAuth()

  // ** States
  const [data, setData] = useState<ShipmentInListType[]>(shipmentsExport)
  ...
  useEffect(() => {
    dispatch(getExports(token))
  }, [])

  useEffect(() => {
    if (shipmentsExport) {
      setData(shipmentsExport)
    }
  }, [shipmentsExport])
  ...
  return (
    <Card>
      ...
      <Stack
        ...
        sx={{ p: 2 }}
      >
        <CardHeader title="My Export Shipments 🚢" subheader="Accessing Data for All Export Shipments" />
...
export default TableShipmentExports
